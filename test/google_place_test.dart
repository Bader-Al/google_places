...
void main() {
  ...
  test('init', () async {
    var googlePlace = GooglePlaces(apiKey, proxyUrl: 'cors-anywhere.herokuapp.com');
  ...
  test('proxies with paths and parameters', () async {
    ...
    googlePlace = GooglePlaces(apiKey, proxyUrl: 'https://localhost:6969/proxy/proxy-two/');
    await googlePlace.search.getFindPlace("some place", InputType.TextQuery);
    googlePlace = GooglePlaces(apiKey, proxyUrl: 'localhost:6969/proxy/proxy-two?gmapsurl=');
    await googlePlace.autocomplete.get("some place");
    //try a bad hostname
    googlePlace = GooglePlaces(apiKey, proxyUrl: 'localhost:6969******');
